...
@software: PyCharm
@file: D14.py
@time: 2018/1/11 0011 上午 11:58
"""

"""
器学习(ML),自然语言处理(NLP),信息检索(IR)等领域,评估(Evaluation)是一个必要的工作,而其评价指标往往有如下几点:准确率(Accuracy),精确率(Precision),召回率(Recall)和F1-Measure。(注：相对来说，IR 的 ground truth 很多时候是一个 Ordered List, 而不是一个 Bool 类型的 Unordered Collection，在都找到的情况下，排在第三名还是第四名损失并不是很大，而排在第一名和第一百名，虽然都是“找到了”，但是意义是不一样的，因此更多可能适用于 MAP 之类评估指标。)
...
精确率(precision)的公式是P = \frac{TP}{TP+FP}P= 
TP+FP
TP
...
召回率(recall)的公式是R = \frac{TP}{TP+FN}R= 
TP+FN
TP
...
F_1 = \frac{2PR}{P+R} = \frac{2TP}{2TP + FP + FN} F 
1
​	 = 
P+R
2PR
...
F_\beta = (\beta^2 + 1) * \frac{PR}{\beta^2*P+R}F 
β
​	 =(β 
2
 +1)∗ 
...
F1-measure 认为精确率和召回率的权重是一样的,但有些场景下,我们可能认为精确率会更加重要, 调整参数 \betaβ , 使用 F_\beta 
β
​	 -measure 可以帮助我们更好的 evaluate 结果.
